<%@include file='master/header.jsp'%>
<div id="divContenedor">
	<div id="divTitulo">Entrega Articulos</div>
	<div id="divBusqueda">
		Buscador por Nombre o Código<br /> <input id="Text7" type="text"
			value="" /> <input id="Submit1" type="submit" value="Buscar"
			class="boton" /><br /> <br />
	</div>
	<div id="divListado" runat="server">
		<div style="padding-bottom: 4px">&nbsp;</div>
		<div style="width: 1016px; height: 300px; overflow: auto;">
			<table class="highLightRow" cellpadding="3" cellspacing="0"
				border="1" width="1000">
				<tr style="background-color: #dfdfdf; height: 25px">
					<th align="left" width="10px"><input id="Checkbox1"
						type="checkbox" /></th>
					<th align="left" width="100px">Codigo</th>
					<th align="left" width="200px">Nombre</th>
					<th align="left" width="150px">Fecha Creaci&oacute;n</th>
					<th align="left" width="150px">Fecha Solicitud</th>
					<th>&nbsp; Cumplida</th>
				</tr>
				<tr>
					<td><input id="Checkbox2" type="checkbox" /></td>
					<td>D237UE</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Image1" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox3" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img1" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox4" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img3" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox5" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"></td>
				</tr>
				<tr>
					<td><input id="Checkbox6" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"></td>
				</tr>
				<tr>
					<td><input id="Checkbox7" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;">&nbsp;</td>
				</tr>
				<tr>
					<td><input id="Checkbox8" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;">&nbsp;</td>
				</tr>
				<tr>
					<td><input id="Checkbox9" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img13" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox10" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;">&nbsp;</td>
				</tr>
				<tr>
					<td><input id="Checkbox11" type="checkbox" /></td>
					<td>11155561</td>
					<td>ProductoNombre0</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img17" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox12" type="checkbox" /></td>
					<td>Nombre</td>
					<td>ProductoNombre</td>
					<td>2013-01-02 09:28:35</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img19" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox13" type="checkbox" /></td>
					<td>Nombre</td>
					<td>ProductoNombre</td>
					<td>&nbsp;</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img21" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox14" type="checkbox" /></td>
					<td>Nombre</td>
					<td>ProductoNombre</td>
					<td>&nbsp;</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img23" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox15" type="checkbox" /></td>
					<td>Nombre</td>
					<td>Apellido</td>
					<td>&nbsp;</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img25" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
				<tr>
					<td><input id="Checkbox16" type="checkbox" /></td>
					<td>Nombre</td>
					<td>Apellido</td>
					<td>&nbsp;</td>
					<td>2013-01-02 09:28:35</td>
					<td style="width: 30px; text-align: center;"><a
						href="?a=edit&id="> <img id="Img27" src="img/tick.png"
							alt="Modificar" border="0" />
					</a></td>
				</tr>
			</table>
		</div>
		<div
			style="padding: 4px; vertical-align: middle; background-color: #e9e9e9;">
			Cantidad de Stock multiple: <input id="Text9" type="text" /> <input
				id="btnNuevo0" type="button" value="Modificar Todos" class="boton" />
		</div>
	</div>
	<div id="divForm" runat="server" style="margin-top: 20px">
		<table id="tablaListado"
			style="width: 500px; border: solid 1px #dfdfdf" visible="false">
			<tr>
				<td colspan="2"
					style="background-color: #dfdfdf; font-weight: bold; height: 30px">
					Edicion de Articulo</td>
			</tr>
			<tr>
				<td width="100px">Codigo</td>
				<td><input id="Text8" type="text" value="D237UE" /></td>
			</tr>
			<tr>
				<td width="100px">Nombre</td>
				<td><input id="Text1" type="text" value="Silla nro3" /></td>
			</tr>
			<tr>
				<td width="100px">Stock</td>
				<td><input id="Text2" type="text" value="4000" /></td>
			</tr>
			<tr>
				<td width="100px">Descripción</td>
				<td><input id="Text3" type="text" value="" /></td>
			</tr>
			<tr>
				<td width="100px">tipo</td>
				<td><select id="Select1">
						<option></option>
						<option>tipo1</option>
						<option selected="selected">tipo2</option>
						<option>tipo3</option>
						<option>tipo4</option>
				</select></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td><br /> <input type="button" id="btnSave" value="Guardar"
					class="boton" /> <br /></td>
			</tr>
		</table>
	</div>
</div>
<%@include file='master/footer.jsp'%>
